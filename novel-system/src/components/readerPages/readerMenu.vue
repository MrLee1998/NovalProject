<template>
  <div class="reader-menu-container">
    ...
    <div v-show="isMenuShow" class="reader-menu-bottom fix">
      <a
        @click="showcatlog"
        class="reader-menu-tabs reader-menu-toc"
        href="javascript:"
        ><mu-icon class="reader-icon-tabs" value="book">目录</mu-icon></a
      >
      <a
        @click="fontmenu"
        class="reader-menu-tabs reader-menu-font"
        href="javascript:"
        ><mu-icon class="reader-icon-tabs" value="settings">设置</mu-icon></a
      >
      <a
        @click="showsources"
        class="reader-menu-tabs reader-menu-source"
        href="javascript:"
        ><mu-icon class="reader-icon-tabs" value="language">换源</mu-icon></a
      >
      <a
        @click="nightmode"
        class="reader-menu-tabs reader-menu-night"
        href="javascript:"
        ><mu-icon class="reader-icon-tabs" :value="isnighticon"
          >日/夜</mu-icon
        ></a
      >
      <div v-show="isFontMenuShow" class="reader-font-set">
        <ul class="reader-font-box">
          <li
            v-for="(Font, index) in font"
            :key="index"
            class="fontbtn"
            :class="[{ buttonfocus: isactivefont[index] }]"
            @click="fontchange(Font.fontvalue, index)"
            :style="{ fontFamily: Font.fontvalue }"
          >
            {{ Font.name }}
          </li>
        </ul>
        <ul class="reader-font-bg">
          <li
            :class="{ readerFontBgActive: isactive[index] }"
            @click="bgchange(color, index)"
...
export default {
  ...
  methods: {
    ...
    nightmode() {
      if (!this.isnight) {
        this.beforecolor = localStorage.getItem("mybgcolor")
          ? localStorage.getItem("mybgcolor")
          : "#f7eee5";
        localStorage.setItem("mybgcolor", "#0f1410");
        this.isnight = true;
        this.isnighticon = "cloud";
        document.getElementById("reader-page-view").style.background =
          "#0f1410";
      } else {
        localStorage.setItem("mybgcolor", this.beforecolor);
        document.getElementById(
          "reader-page-view"
        ).style.background = this.beforecolor;
        this.isnight = false;
...
.reader-menu-tabs,
.reader-menu-tabs:visited,
.reader-menu-tabs:hover {
  float: left;
  width: 25%;
  text-align: center;
  line-height: 18px;
